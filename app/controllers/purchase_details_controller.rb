class PurchaseDetailsController < ApplicationController
  ...
  def index
    ...
    @total_amount = @purchase_details.to_a.sum { |item| item.item_price }
  end
  ...
  private
    ...
    def purchase_detail_params
      params.require(:purchase_detail).permit(:purchaser_name, :item_description, :item_price, :purchase_count, :merchant_address, :merchant_name)
    end
end
